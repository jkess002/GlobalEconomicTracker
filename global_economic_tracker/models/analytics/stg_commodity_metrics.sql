{{ config(
    materialized='table',
    post_hook=[
        "CREATE INDEX IF NOT EXISTS idx_commodity_ticker_ts ON {{ this }}(ticker, timestamp)"
    ]
) }}

with base as (select *,
                     row_number() over (partition by ticker order by timestamp) as rn
              from {{ ref('stg_commodity_returns') }}),

     rolling as (select ticker,
    timestamp, avg (daily_return) over (
    partition by ticker
    order by rn
    rows between 19 preceding and current row
    ) as rolling_avg_mo, avg (daily_return) OVER (
    partition by ticker
    order by timestamp
    rows between 6 preceding and current row
    ) as weekly_avg_return, stddev(daily_return) over (
    partition by ticker
    order by rn
    rows between 19 preceding and current row
    ) as rolling_volatility_mo, case
